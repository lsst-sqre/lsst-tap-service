...
public class DefaultTableWriter implements TableWriter {
    ...
    private void initFormat() {
        ...
        // Note: This needs to be done before the write method is called so the
        // contentType
                ...
                try {
                    ...
                    if (reqStandardID.equals(standardID)) {
                        if (accessURL == null && resourceIdentifier != null && standardID != null) {
                            ...
                            log.debug("resourceIdentifier=" + resourceIdentifier + ", standardID=" + standardID
                                    + ", authMethod=" + cur);
    ...
    /**
     * Optionally add meta resources to the VOTableDocument. These are expected to
     * be
     * DataLink service descriptors that go with the columns (fields) in the select
     * list.
     * Normally, fields get an ID if one is assigned in the tap_schema metadata. The
     * default
     * implementation uses the <code>fieldIDs</code> valeus to find service
     * descriptor templates,
     * optionally process them to add the accessURL, and add them to the document.
     * Find is currently: look for a file named {fieldID}.xml in the
     * {user.home}/config
     * directory.
     * 
     * @param votableDocument the document to add meta resources to
     * @param fieldIDs        list of FIELD ID attributes for items in the select
     *                        list

...
import org.opencadc.tap.upload.UploadPartitionDetector;
import org.opencadc.tap.util.DatabaseNameUtil;
import org.opencadc.tap.storage.StorageUtils;
import org.opencadc.tap.execution.kafka.messages.JobRun.UploadTable;
import org.opencadc.tap.storage.OutputFormat;
...
public class KafkaJobService {
    ...
    private static final int DEFAULT_JOB_RESULT_EXPIRATION_MINUTES =
            (int) (TapConfig.maxExecutionDurationSeconds() / 60)
            + RESULT_DELIVERY_BUFFER_MINUTES;

    private static final UploadPartitionDetector PARTITION_DETECTOR =
            new UploadPartitionDetector(TapConfig.uploadPartitionDirectors());
                ...
                if (!queryRunner.uploadTableLocations.isEmpty()) {
                    ...
                    for (Map.Entry<String, TableDesc.TableLocationInfo> entry : queryRunner.uploadTableLocations
                        ...
                        if (pi != null) {
                            ...
                            uploadTable.setPartitionType(pi.type.name().toLowerCase());

...
import org.opencadc.tap.ws.WebAppContext;
...
import org.opencadc.tap.execution.kafka.services.JobStatusListener;
import org.opencadc.tap.execution.kafka.services.ReadJobStatus;
    ...
    public void contextInitialized(ServletContextEvent sce) {
        ...
        try {
            if (!Backend.current().usesKafka()) {
                log.info("backend " + Backend.current() + " does not use Kafka, skipping Kafka initialization");
                return;
            }
            ...
            // configuration / initialization errors are fatal: fail context
            // startup rather than come up with a broken async query path
            log.error("Failed to initialize Kafka services", e);
            throw e;
        } catch (Exception e) {
            log.error("Failed to initialize Kafka services", e);
            throw new IllegalStateException("Failed to initialize Kafka services", e);
        }
    }

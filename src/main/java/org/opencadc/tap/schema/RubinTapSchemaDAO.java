...
/**
 * RubinTapSchemaDAO extends the base TapSchemaDAO with Rubin/LSST-specific
 * function whitelists. The standard SQL helpers are advertised for every
 * backend; the QServ spatial and SciSQL UDFs are only advertised when the
 * service is actually backed by QServ.
 */
public class RubinTapSchemaDAO extends TapSchemaDAO {
    private static final Logger log = Logger.getLogger(RubinTapSchemaDAO.class);
    /**
    ...
    protected List<FunctionDesc> getFunctionDescs() {
        ...
        // QServ spatial and SciSQL UDFs are only available on a QServ backend.
        if (Backend.currentOrDefault(Backend.QSERV) != Backend.QSERV) {
            log.debug("non-QServ backend, skipping QServ/SciSQL function whitelist");
